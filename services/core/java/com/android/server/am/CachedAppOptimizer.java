...
public final class CachedAppOptimizer {
    ...
    static final long FORCE_FREEZER_DEBOUNCE_TIMEOUT = 100L;
    static final int FORCE_FREEZER_MIN_ADJ = ProcessList.PERCEPTIBLE_APP_ADJ;
    ...
    MainProcessFilter mMainProcessFilter = new MainProcessFilter();
    ...
    void unscheduleFreezeAppLSP(ProcessRecord app) {
        if (mUseFreezer) {
            ...
            mMainProcessFilter.clearDeadAdj(app);
        }
    }
    ...
    static final class MainProcessFilter {
        HashMap<String, Integer> mMainProcessAdj = new HashMap<>();

        private boolean isMainProcess(String processName) {
            return !processName.contains(":");
        }

        public void reportAdjChange(ProcessRecord app) {
            if (!isMainProcess(app.processName))
                return;
            // Do not treat it as a real main process if it does not have ui.
            // It is more like a background startup.
            if (!app.mState.hasShownUi())
                return;
            mMainProcessAdj.put(app.info.packageName, app.mState.getSetAdj());
        }

        public int getMainProcessAdj(ProcessRecord app) {
            // A fast path for performance, we don't need to search the map
            // for a main process.
            if (isMainProcess(app.processName)) {
                if (app.mState.hasShownUi())
                    return app.mState.getSetAdj();
                else
                    return FORCE_FREEZER_MIN_ADJ;
            }
            // Freeze it if it does not have a valid main process
            return mMainProcessAdj.getOrDefault(app.info.packageName, FORCE_FREEZER_MIN_ADJ);
        }

        public void clearDeadAdj(ProcessRecord app) {
            if (!isMainProcess(app.processName))
                return;
            mMainProcessAdj.remove(app.info.packageName);
        }
    }
